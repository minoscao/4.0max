...
- Operator and technician List → Detail pages must start with no selected task; task details and task-specific actions appear only after the worker deliberately selects a work order. Current and history pages render the work-order list only—never an inline detail pane or reserved empty detail area. Clicking a row opens a separate second-level drawer/sheet from the bottom on mobile, and closing it clears the selection.
